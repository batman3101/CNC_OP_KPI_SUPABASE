...
def save_worker_data(worker):
    try:
        ...
        if success:
            st.success(translate("작업자 데이터가 저장되었습니다."))
        else:
            st.error(translate("작업자 데이터 저장 중 오류가 발생했습니다."))
...
def update_worker_data(old_name, new_name, new_id, new_line):
    try:
        ...
        if not st.session_state.db.client:
            ...
            st.error(translate("데이터베이스 연결 오류. 관리자에게 문의하세요."))
            return False
        ...
        if success:
            ...
            st.success(f"{translate('작업자')} '{old_name}'{translate('의 정보가 업데이트되었습니다.')}")
            ...
            st.error(f"{translate('작업자')} '{old_name}' {translate('데이터 업데이트에 실패했습니다.')}")
            return False
            
    except Exception as e:
        print(f"[ERROR] 작업자 업데이트 중 예외 발생: {e}")
        st.error(f"{translate('작업자 정보 업데이트 중 오류가 발생했습니다')}: {str(e)}")
        import traceback
...
def delete_worker_data(worker_name):
    try:
        ...
        if success:
            ...
            st.success(f"{translate('작업자')} '{worker_name}'{translate('이(가) 삭제되었습니다.')}")
            ...
            st.error(f"{translate('작업자')} '{worker_name}' {translate('삭제에 실패했습니다.')}")
            return False
            
    except Exception as e:
        print(f"[ERROR] 작업자 삭제 중 예외 발생: {e}")
        st.error(f"{translate('작업자 삭제 중 오류가 발생했습니다')}: {str(e)}")
        import traceback
...
def show_worker_management():
    ...
    # 작업자 데이터 로드 버튼
    if st.button(translate("🔄 데이터 새로고침"), key="refresh_all", use_container_width=True):
        ...
        st.success(translate("작업자 데이터를 새로고침했습니다."))
        st.rerun()  # 페이지 리로드
    
    # 작업자 데이터 항상 최신으로 로드
    if 'workers' not in st.session_state:
        print("[INFO] 초기 작업자 데이터 로드")
        with st.spinner(translate("작업자 데이터 로드 중...")):
            st.session_state.workers = load_worker_data()
    ...
    # 작업자 목록 탭
    with tab1:
        st.subheader(translate("등록된 작업자 명단"))
        # 데이터 새로고침 버튼
        if st.button(translate("새로고침"), key="refresh_workers"):
            with st.spinner(translate("작업자 데이터 로드 중...")):
                st.session_state.workers = load_worker_data()
                st.success(translate("작업자 목록을 새로고침했습니다."))
                st.rerun()
        ...
        if st.session_state.workers:
            ...
            st.info(translate("등록된 작업자가 없습니다. '신규 등록' 탭에서 작업자를 추가해주세요."))
    
    # 신규 등록 탭
    with tab2:
        st.subheader(translate("신규 작업자 등록"))
        with st.form("worker_registration_form"):
            new_id = st.text_input(translate("사번"), placeholder=translate("예: 21020147"))
            new_name = st.text_input(translate("이름"), placeholder=translate("예: DƯƠNG THỊ BỒNG"))
            new_dept = st.text_input(translate("부서"), value="CNC")
            new_line = st.text_input(translate("라인번호"), placeholder=translate("예: B-200"))
            
            submit_button = st.form_submit_button(translate("등록"), use_container_width=True)
            ...
            if submit_button:
                if not new_id or not new_name or not new_dept or not new_line:
                    ...
                    if any(w.get("사번") == new_id for w in st.session_state.workers):
                        st.error(translate("이미 등록된 사번입니다."))
                    else:
                        new_worker = {
                            "사번": new_id,
                            "이름": new_name,
                        ...
                        with st.spinner(translate("작업자 등록 중...")):
                            if save_worker_data(new_worker):
                                # 작업자 목록 즉시 새로고침
                                st.session_state.workers = load_worker_data()
                                st.success(f"{translate('작업자')} '{new_name}'{translate('이(가) 등록되었습니다.')}")
                                st.rerun()
    
    # 수정/삭제 탭
    with tab3:
        st.subheader(translate("작업자 정보 수정/삭제"))
        
        if not st.session_state.workers or len(st.session_state.workers) == 0:
            st.info(translate("수정/삭제할 작업자가 없습니다. '신규 등록' 탭에서 작업자를 추가해주세요."))
            return
        
        # 작업자 목록 다시 로드하는 버튼 추가
        if st.button(translate("작업자 목록 새로고침"), key="reload_worker_list"):
            with st.spinner(translate("작업자 데이터 로드 중...")):
                # 캐시 무효화 먼저 수행
                st.session_state.db._invalidate_cache()
        ...
        # 현재 목록 출력 - 디버깅 및 확인용
        st.write(f"### {translate('현재 작업자 목록')}")
        if st.session_state.workers:
            df = pd.DataFrame(st.session_state.workers)
            # 필요한 열만 선택
            display_columns = ['사번', '이름', '부서', '라인번호']
            ...
            if len(worker_options) > 0:
                ...
                if selected_worker:
                    ...
                    # 현재 작업자 정보 표시
                    st.write(f"#### {translate('현재 작업자 정보')}")
                    info_col1, info_col2 = st.columns(2)
                    with info_col1:
                        st.write(f"**{translate('사번')}**: {worker.get('사번', '')}")
                        st.write(f"**{translate('이름')}**: {worker.get('이름', '')}")
                    with info_col2:
                        st.write(f"**{translate('부서')}**: {worker.get('부서', 'CNC')}")
                        st.write(f"**{translate('라인번호')}**: {worker.get('라인번호', '')}")
                    
                    st.write("---")
                    
                    # 수정 폼
                    st.write(f"#### {translate('작업자 정보 수정')}")
                    
                    with st.form(key="edit_worker_form"):
                        # 입력 필드 정의
                        edit_id = st.text_input(translate("사번"), value=worker.get("사번", ""), key="edit_id")
                        edit_name = st.text_input(translate("이름"), value=worker.get("이름", ""), key="edit_name")
                        edit_dept = "CNC"  # 부서는 항상 CNC로 고정
                        edit_line = st.text_input(translate("라인번호"), value=worker.get("라인번호", ""), key="edit_line")
                        
                        # 저장 버튼
                        save_button = st.form_submit_button(translate("저장"), use_container_width=True)
                        ...
                        if save_button:
                            ...
                            if (edit_id == worker.get("사번", "") and 
                                ...
                                st.warning(translate("변경된 내용이 없습니다."))
                            else:
                                # 필수 입력 확인
                                if not edit_id or not edit_name or not edit_line:
                                    ...
                                    # 작업자 정보 업데이트
                                    with st.spinner(f"{translate('작업자')} '{original_name}' {translate('정보 업데이트 중...')}"):
                                        if update_worker_data(original_name, edit_name, edit_id, edit_line):
                                            st.success(f"{translate('작업자')} '{original_name}'{translate('의 정보가 업데이트되었습니다.')}")
                    
                    # 삭제 영역 - 폼 외부에 배치
                    st.write(f"#### {translate('작업자 삭제')}")
                    if st.button(translate("작업자 삭제"), key="delete_worker_btn", type="primary"):
                        # 세션 상태에 삭제 모드 저장
                    ...
                    if st.session_state.get("delete_mode", False):
                        st.warning(f"{translate('작업자')} **{st.session_state.delete_worker_name}**{translate('을(를) 정말 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')}")
                        
                        # 삭제 확인 폼
                        with st.form(key="confirm_delete_form"):
                            # 확인을 위한 작업자 이름 입력
                            confirm_name = st.text_input(translate("삭제를 확인하려면 작업자 이름을 입력하세요:"), key="confirm_delete")
                            
                            col1, col2 = st.columns(2)
                            with col1:
                                confirm_button = st.form_submit_button(translate("확인 삭제"), type="primary")
                            with col2:
                                cancel_button = st.form_submit_button(translate("취소"))
                            
                            if confirm_button:
                                if confirm_name == st.session_state.delete_worker_name:
                                    # 작업자 삭제
                                    with st.spinner(f"{translate('작업자')} '{st.session_state.delete_worker_name}' {translate('삭제 중...')}"):
                                        if delete_worker_data(st.session_state.delete_worker_name):
                                            st.success(f"{translate('작업자')} '{st.session_state.delete_worker_name}'{translate('이(가) 삭제되었습니다.')}")
                                            # 삭제 모드 해제
                                            st.session_state.delete_mode = False
                                            st.session_state.delete_worker_name = ""
                                            # 작업자 목록 새로고침
                                    ...
                                    st.error(translate("입력한 이름이 일치하지 않습니다."))
...
def show_worker_history():
    st.info(translate("작업자 이력 관리 기능은 준비 중입니다."))
